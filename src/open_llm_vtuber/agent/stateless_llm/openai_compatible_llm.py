...
def _filter_female_text(text: str) -> str:
    ...
    async def chat_completion(
        ...
        try:
            ...
            async for chunk in stream:
                ...
                content = _filter_female_text(content)
                if content:
                    temp_text = accumulated_text + content
                    patterns = ["Human:", "AI:", "Human :", "AI :", "\nHuman", "\nAI", "Instruct:", "Instruction:", "###", "[Interrupted", "\nInstruct"]
                    earliest_pos = len(temp_text)
                    pattern_found = False
                    for pattern in patterns:
                        pos = temp_text.find(pattern)
                        if pos != -1 and pos < earliest_pos:
                            earliest_pos = pos
                            pattern_found = True

                    if pattern_found:
                        needed_len = earliest_pos - len(accumulated_text)
                        if needed_len > 0:
                            yield content[:needed_len]
                        logger.info(f"Cut off dialogue script at index {earliest_pos}")
                        break
